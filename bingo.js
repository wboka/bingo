...
var bingo = {
	...
	hasBingo: function() {
		...
		for (var key in possibleBingos) {
			if ((key.toLowerCase() === "corners" && possibleBingos[key].length === 4) || possibleBingos[key].length === 5) {
				$card.find(".call-bingo").after("<div class='alert alert-success text-center'><h1>BINGO!!!</h1>You got BINGO after " + bingo.calledNumbers.length + " balls were called.</div>");
	...
	play: function() {
		...
		timer = setInterval(function() {
			...
			if (bingo.availableNumbers.length === bingo.calledNumbers.length) { 
				clearInterval(timer);
